...
function parseExpression(expr) {
  ...
  switch (operator) {
    case undefined:
      return key[0] === '!' ? new Rule(key.slice(1), Operator.DoesNotExist) : new Rule(key, Operator.Exists)
    case '=':
    case '==':
      return new Rule(key, Operator.Equals, [value])
    case '!=':
      return new Rule(key, Operator.NotEquals, [value])
    case 'in':
      return new Rule(key, Operator.In, values)
    case 'notin':
      return new Rule(key, Operator.NotIn, values)
    case '>':
      return new Rule(key, Operator.GreaterThen, [value])
    case '<':
      return new Rule(key, Operator.LessThen, [value])
...
function parseSelector(labelSelector) {
  const expressions = labelSelector.split(/,(?![^(]*\))/).map(parseExpression)
  ...
  const matchExpressions = expressions.filter((expr) => !hasOwnProperty(expr, 'value'))

  return {matchLabels, matchExpressions}
}

function parseSelectorExpression(labelSelector) {
  return labelSelector.split(/,(?![^(]*\))/).map(parseExpression)
}
...
function isExprMatch(expr, labels) {
  ...
  switch (op) {
    case 'Exists':
      return hasOwnProperty(labels, key)
    case 'DoesNotExist':
      return !hasOwnProperty(labels, key)
    case 'In':
...
function Selector(opts) {
  ...
  return (labels) => expressions.every((expr) => isExprMatch(expr, labels || {}))
}

export {Rule, Operator, stringify, parseSelector, parseSelectorExpression, Selector}
